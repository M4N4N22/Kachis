"use client";

import { Fingerprint } from "lucide-react";
import { Bento } from "@/components/ui/bento";
import { Button } from "@/components/ui/button";
import { useApp } from "@/lib/app-store";

const CREDENTIALS = [
  {
    title: "Employment",
    issuer: "Northwind Capital",
    disclosed: "Title only",
  },
  {
    title: "Org membership",
    issuer: "Security desk",
    disclosed: "Seat + tier",
  },
  {
    title: "Compliance officer",
    issuer: "Internal GRC",
    disclosed: "Boolean eligible",
  },
];

export function IdentityView() {
  const { profile, tier, wallet } = useApp();
  const connected = wallet.status === "connected";

  return (
    <div className="grid gap-4 lg:grid-cols-[minmax(0,20rem)_1fr]">
      <Bento className="relative overflow-hidden p-6">
        <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_top_right,color-mix(in_srgb,var(--brand)_22%,transparent),transparent_46%)]" />
        <div className="relative">
          <p className="text-[11px] font-semibold text-brand">Midnight Passport</p>
          <h2 className="mt-6 text-2xl font-light tracking-tight">{profile.name}</h2>
          <p className="mt-1 text-[13px] text-muted-fg">
            {profile.organization} · {profile.title}
          </p>
          <div className="mt-8 flex items-center gap-2 text-[11px] text-muted-fg">
            <Fingerprint className="h-3.5 w-3.5 text-brand" strokeWidth={1.75} />
            {connected
              ? `Bound to ${wallet.provider === "lace" ? "Lace" : "Gero"}`
              : "Wallet not bound"}
          </div>
          <p className="mt-6 font-mono text-[11px] text-muted-fg">
            {wallet.address ?? "addr_midnight1 · pending"}
          </p>
        </div>
      </Bento>

      <div className="space-y-4">
        <Bento className="p-5">
          <p className="text-[11px] font-semibold text-brand">Selective disclosure</p>
          <h2 className="mt-1 text-sm font-semibold tracking-tight">
            Prove a fact, keep the rest
          </h2>
          <div className="mt-5 grid gap-3 sm:grid-cols-3">
            {(tier === "freelancer" ? CREDENTIALS.slice(0, 1) : CREDENTIALS).map(
              (credential) => (
                <div key={credential.title} className="rounded-2xl bg-bg p-4">
                  <p className="text-[13px] font-medium">{credential.title}</p>
                  <p className="mt-1 text-[11px] text-muted-fg">{credential.issuer}</p>
                  <p className="mt-4 text-[11px] text-brand">{credential.disclosed}</p>
                </div>
              ),
            )}
          </div>
          <Button variant="outline" size="sm" className="mt-5">
            Disclose selected claims
          </Button>
        </Bento>

        <Bento className="p-5">
          <p className="text-[11px] font-semibold text-brand">Integration placeholder</p>
          <p className="mt-2 max-w-xl text-[13px] leading-6 text-muted-fg">
            Compact credential circuits and Midnight proof-server handshakes land
            next. This desk already models the disclosure UX judges will click
            through.
          </p>
        </Bento>
      </div>
    </div>
  );
}
